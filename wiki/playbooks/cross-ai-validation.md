...
layer: wiki
language: en-es
tags: [cross-ai, validation, workflow, decision-making, audit, playbook, capa/2-wiki]
updated: 2026-04-24
...
| **Claude Opus 4.7** (skill cross-validate substitute por Grok) | Repo-context coherence checker | **Most consistent long-context performance of any model tested** (0.715 score). Best at "drawing connections across documents" + "precise attribution". Único validator con acceso completo al repo (decisions, otros archivos, playbooks, rules) | Sesgo del autor — Opus puede haber escrito el archivo que valida; debe declarar y compensar | [claude-prompt-template.md](../../.claude/skills/cross-validate/templates/claude-prompt-template.md) |

**Calibration rule:** weight findings by each AI's actual signal
quality, not its prestige. A Gemini technical finding (specific,
measurable, structural) can outweigh three AIs' stylistic consensus.
...
[.claude/skills/cross-validate/templates/](../../.claude/skills/cross-validate/templates/)
...
detección de drift. La columna "Operationalized by" arriba es el ancla
de cross-ref — mantener actualizada cuando se agreguen o reemplacen
templates.
...
con acceso al repo completo — chequea coherencia del archivo validado
con decisions del wiki, otros archivos del repo, playbooks, framework
...
**Claude.ai Project** (where project context is already loaded):
...
Claude Code's job is files and git. Strategy → chat; File operations
→ Claude Code.
...
1. **Frontmatter** — `type: audit`, `status: station`, `project:`,
   `related_paths:` pointing at the master/source file + this playbook.
2. **Imperative opening** — the first prose after the H1 title. State
   the action: "Generá 4 prompts cross-AI para [decisión X]. Cada
   prompt debe [...]. Luego sintetizá los 4 verdicts en un veredicto
   único siguiendo la estructura del playbook." This is the only
   instruction in the file. Do not duplicate it elsewhere.
...
"<filename>.md §<N> <element>: replace 'X' with 'Y'"
...
- **Pattern-breaking for preference** — stylistic swaps framed as
  evidence-based when they're just preference.
- **Resume vs posture** — AIs optimize for semantic density
  (keyword-dense bio); brands often win on posture (shorter, more
  confident bio matching other-surface copy).
...
- Schema: frontmatter rules in [.claude/rules/frontmatter.md](../../.claude/rules/frontmatter.md)
- Workflow: [.claude/commands/safe-change.md](../../.claude/commands/safe-change.md) applies the synthesized verdict
- Skill que automatiza este playbook end-to-end: [.claude/skills/cross-validate.md](../../.claude/skills/cross-validate.md)
