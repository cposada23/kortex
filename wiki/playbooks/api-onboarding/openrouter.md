...
layer: wiki
language: es
tags: [capa/2-wiki, api, openrouter, perplexity, cross-validate, onboarding]
updated: 2026-04-24
confidence: high
source_count: 5
...
## Por qué OpenRouter en el stack del template

**Primario:** acceso a **Perplexity Sonar Pro** — la API directa de
Perplexity pide un mínimo alto al sign-up (observado ~$50 USD),
incompatible con presupuestos chicos para testing del skill. Vía
...
**Resultado:** cuando ese key alcance el cap en el período, OpenRouter
...
Estado al 2026-04-24 (verificar [openrouter.ai/perplexity](https://openrouter.ai/perplexity)):
...
El cap mensual del skill se configura por vos (default sugerido del
template: $3 USD para Perplexity dentro de un cap total de $10 split
entre los 3 proveedores externos; ajustá según tu uso real).

Protecciones apiladas:
1. **Budget cap del key en OpenRouter** (tu monto/mes) — hard stop a
   nivel gateway.
2. **Saldo cargado** (lo que cargaste) — hard stop a nivel cuenta.
3. **Cap del skill** (configurable, tracked en `output/costs/cross-validate-log.md`)
...
**Por default el template no usa esto** — OpenAI y Gemini se llaman
directo. BYOK sólo aplica si alguna vez querés centralizar todos los
gateways en una sola interfaz.
