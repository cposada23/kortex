...
layer: wiki
language: es
tags: [capa/2-wiki, api, gemini, google, cross-validate, onboarding]
updated: 2026-04-24
confidence: high
source_count: 4
...
| **Gemini 2.5 Flash** | ✅ Free con quota baseline más alta | Testing rápido del skill — flag `--gemini-model gemini-2.5-flash` |
...
para los caps actuales. Al 2026-04-24 alcanzan cómodo para ~50-100
...
Para cross-validate con cap chico el spend cap nuevo alcanza.
...
template: $3 USD para Gemini dentro de un cap total de $10 split entre
los 3 proveedores externos; ajustá según tu uso real). Protección
adicional: el project-level spend cap de $10 que seteás en AI Studio
(sección 4.2) actúa como segundo gate duro.

**Por modo de uso:**
- **Testing del skill** (flag `--gemini-model gemini-2.5-flash`) → $0,
  corre en free tier. Usarlo mientras se itera sobre el prompt /
  scoring / format del skill.
- **Validación real** (default `--gemini-model gemini-3.1-pro`) → paid
  tier. Con el cap del skill + spend cap de proyecto, ambos límites son
  seguros para testing extenso del primer mes.
